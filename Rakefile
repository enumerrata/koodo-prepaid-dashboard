...
#https://gist.github.com/wbailey/822095#file-databases-rake
namespace :db do
  ...
  task :configuration => :environment do
    @config = YAML.load(ERB.new(File.read(File.join('config', 'database.yml'))).result)[RACK_ENV]
  end
  ...
  desc 'Create the database from config/database.yml for the current RACK_ENV'
